...
// Initialize a GPIO port and at the same time
// start the clock controlling the port
...
// Write a byte to the upper or lower byte of a 16 bit port

void write_GPIO_Byte(GPIO_TypeDef* GPIOx,
                     uint8_t high_low_byte,
					      uint8_t byte_value);

// Read a byte from the upper or lower byte of a 16 bit port
